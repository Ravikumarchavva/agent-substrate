...
async def test_unknown_type_resolves_to_none() -> None:
    ...
    r = ActorResolver()
    assert await r.resolve(Actor(type="conversation", key="s1")) is None


async def test_factory_activates_on_demand() -> None:
    r = ActorResolver()
    r.register_factory("conversation", lambda a: StubAgent(a))

    agent = await r.resolve(Actor(type="conversation", key="s1"))
...
async def test_async_factory_is_awaited() -> None:
    """A factory that does real async work (acquire a bridge, load state)
    must be supported — building a real agent usually isn't synchronous."""

    async def factory(a: Actor) -> StubAgent:
        return StubAgent(a)
...
async def test_one_factory_serves_many_instances() -> None:
    ...
    for i in range(1_000):
        await r.resolve(Actor(type="conversation", key=f"s{i}"))

    assert len(built) == 1_000
    assert r.live_count == 1_000


async def test_repeat_resolve_reuses_the_same_instance() -> None:
    builds = 0
    ...
    first = await r.resolve(addr)
    second = await r.resolve(addr)
...
async def test_evicted_actor_is_rebuilt_on_next_resolve() -> None:
...
async def test_registered_instance_is_never_evicted() -> None:
    """register() pins by default — a singleton must not vanish under load."""
    r = ActorResolver(max_live=2, idle_ttl=0)
    ...
    assert await r.resolve(Actor(type="http_proxy")) is pinned


async def test_unpinned_instance_is_evictable() -> None:
    """The virtual-actor migration path: a caller that already builds a
    fresh instance per entity on every call doesn't need a factory at all —
    registering with pinned=False is enough to stop it being held forever."""
    r = ActorResolver(max_live=2, idle_ttl=0)
    addr = Actor(type="assistant", key="thread-1")
...
async def test_factories_are_selected_by_type() -> None:
    ...
    conv = await r.resolve(Actor(type="conversation", key="s1"))
    analyst = await r.resolve(Actor(type="data_analyst", key="s1"))
